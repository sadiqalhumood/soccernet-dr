...
def load_spotting_annotations(labels_json: Path) -> dict:
    ...
    Includes both 'visible' and 'not shown' to match SoccerNet-v2 evaluation protocol.
...
def is_hit_multi(predictions: list, gt_list: list, delta_ms: float, k: int) -> bool:
    """
    True if any of the top-k (half, pos_ms) predictions is within delta_ms
    of any (half, pos_ms) ground truth. Used for action spotting with multiple GTs.
    """
    for pred_half, pred_ms in predictions[:k]:
        for gt_half, gt_ms in gt_list:
            if pred_half == gt_half and abs(pred_ms - gt_ms) <= delta_ms:
                return True
    return False
